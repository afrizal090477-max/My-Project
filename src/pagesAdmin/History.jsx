...
import { FaRegEdit } from "react-icons/fa"; // Icon Action
...
export default function History() {
  const [history] = useState(dummyData); // Data table hanya state for render
          ...
          {filteredData.map((item) => (
            <tr key={item.id} className="even:bg-gray-50">
              ...
              <td className="py-2 px-3">
                <span
                  className={
                    "px-3 py-1 rounded-full font-bold text-white " + statusColors[item.status]
                  }
                >
                  {item.status}
                </span>
